...
buttonRandomColor.addEventListener('click', () => {
  for (let index = 1; index < palleteElement.length; index += 1) {
    const randomRGB = `rgb(${parseInt(Math.random() * 255)},${parseInt(Math.random() * 255)},${parseInt(Math.random() * 255)})`;
...
for (let index = 0; index < pixel.length; index += 1) {
  pixel[index].addEventListener('click', (target) => {
    ...
    if ((target.target.style.backgroundColor === 'white') && (selectedColor.length !== 0)) {
      target.target.style.backgroundColor = selectedColor[0].style.backgroundColor;
    } else if (selectedColor.length === 0) {
      target.target.style.backgroundColor = 'white';
    } else { target.target.style.backgroundColor = selectedColor[0].style.backgroundColor; }
    attPixelBoard();
  });
}
